...
# We import the needed python libraries
	...
	for i in range(0,len(Ind)):    
		...
		try:
			...
			if D_Vect_Day[Num]>D_Vect_Night[Num]:
				T_mix_Night[x,:]  = T_Vect_Night[Num,:]
				S_mix_Night[x,:]  = S_Vect_Night[Num,:]
				Err[x,0:nperms]  = D_Vect[:].T
				S_mix_Day[x,:]  = S_mix_Night[x,:]
				Cc=np.where(S_mix_Night[x,:]!=0)[0]
				[DD,TT] = trust_cest_DN(S_mix_Day[x,Cc], Im2D_Day[x,:], Emiss_Day[:,Cc], T_mean_Day[:,Cc], Atmo_Day, 'gamma', gamma,'nargout',1)
				T_mix_Day[x,Cc]  = TT
				print('Re-computing Day',S_mix_Day[x,Cc],TT)
			elif D_Vect_Day[Num]<D_Vect_Night[Num]:
				T_mix_Day[x,:]  = T_Vect_Day[Num,:]
				S_mix_Day[x,:]  = S_Vect_Day[Num,:]
				S_mix_Night[x,:]  = S_mix_Day[x,:]
				Err[x,0:nperms]  = D_Vect[:].T
				Cc=np.where(S_mix_Day[x,:]!=0)[0]
				[DD,TT] = trust_cest_DN(S_mix_Night[x,Cc], Im2D_Night[x,:], Emiss_Night[:,Cc], T_mean_Night[:,Cc], Atmo_Night, 'gamma', gamma,'nargout',1)
				T_mix_Night[x,Cc]  = TT
				print('Re-computing Night',S_mix_Night[x,Cc],TT)
		except:
			print('# = %d\n',i) 
			T_mix_Day[x,:]=np.nan 
	...
	if nargout>4:                                
		varargout = Err
		return S_mix_Day, T_mix_Day,T_mix_Night,varargout
	return S_mix_Day, T_mix_Day,T_mix_Night
